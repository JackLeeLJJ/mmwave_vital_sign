...
# 生成对齐1024维度的数据集
data,labels=loadData(256,train_wake_radar_dir,train_ecg_dir)
...
class VED(nn.Module):
    def __init__(self, input_dim=256, latent_dim=65):
        super().__init__()

        # 编码器结构
        self.encoder = nn.Sequential(
            nn.Conv1d(1, 4, kernel_size=5, stride=4, padding=2),  # 256 → 64
            nn.BatchNorm1d(4),
            nn.LeakyReLU(0.2),
            nn.Conv1d(4, 8, 5, 4, 2),  # 64 → 16
            nn.BatchNorm1d(8),
            nn.LeakyReLU(0.2),
            nn.Conv1d(8, 16, 5, 4, 2),  # 16 → 4
            nn.BatchNorm1d(16),
            nn.LeakyReLU(0.2),
            nn.Flatten(),
            nn.Linear(16 * (input_dim // (4 ** 3)), 256)  # 16 * 4=64 → 256
        )
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, 16 * (input_dim // (4 ** 3))),  # 64 → 64
            nn.Unflatten(1, (16, input_dim // (4 ** 3))),  # → [32,4]
...
model = VED(input_dim=256).to(device)
