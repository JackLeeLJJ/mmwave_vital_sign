...
# 生成对齐1024维度的数据集
data,labels=loadData(64,train_wake_radar_dir,train_ecg_dir)
...
class VED(nn.Module):
    def __init__(self, input_dim=64, latent_dim=128):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(8 * (input_dim // (4 ** 2)), 256)  # 16 * 4=64 → 256
        )
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, 8 * (input_dim // (4 ** 2))),
            nn.Unflatten(1, (8, input_dim // (4 ** 2))),
...
model = VED(input_dim=64).to(device)
