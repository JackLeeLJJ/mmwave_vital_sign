...
# 1. 模拟数据生成（重新设计合成数据，长度为4096）
def generate_synthetic_data(num_samples=1000, seq_length=4096):
    """生成包含心跳和呼吸信号的合成数据"""
    # 心跳信号（模拟BVP波形）
...
for _ in range(num_samples):
    ...
    i, q, h = generate_synthetic_data(seq_length=4096)
...
class VED(nn.Module):
    ...
    def _build_encoder(self, input_dim):
        return nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Flatten(),
            nn.Linear(64 * (input_dim // (4 ** 5)), 256)  # 假设最终要映射到256维
        )
    ...
    def _build_decoder(self, latent_dim, input_dim):
        return nn.Sequential(
            ...
            nn.ConvTranspose1d(64, 32, 5, 4, 2, output_padding=3),
            nn.BatchNorm1d(32),
            nn.LeakyReLU(0.2),

            nn.ConvTranspose1d(32, 16, 5, 4, 2, output_padding=3),
            nn.BatchNorm1d(16),
            nn.LeakyReLU(0.2),

            nn.ConvTranspose1d(16, 8, 5, 4, 2, output_padding=3),
            nn.BatchNorm1d(8),
            nn.LeakyReLU(0.2),

            nn.ConvTranspose1d(8, 4, 5, 4, 2, output_padding=3),
            nn.BatchNorm1d(4),
            nn.LeakyReLU(0.2),

            # 新增第五层转置卷积
            nn.ConvTranspose1d(4, 1, 5, 4, 2, output_padding=3),  # 直接输出单通道
            nn.Tanh()
        )
...
def plot_results():
    ...
    plt.plot(sample_h[0].cpu().numpy(), label="Ground Truth")
