...
class VED(nn.Module):
    def __init__(self, input_dim=1024, latent_dim=64):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, 32 * (input_dim // (4 ** 4))),  # 256 → 32 * 4=128
    ...
    def encode(self, x):
        ...
        return mu, logvar  # 返回两个值

    def forward(self, x):
        mu, logvar = self.encode(x)  # 现在可以正确解包
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        # 多目标损失
        loss = loss_function(reconstructions, targets,logvar,mu)
        loss.backward()
...
def visualize_comparison():
    ...
    # 增加图形高度并调整布局参数
    plt.figure(figsize=(15, 8))  # 原为(15,6)，增加高度
    ...
    for i in range(2):
        ...
        # 优化图例和标题样式
        plt.legend(loc='upper right', fontsize=8)  # 缩小图例字体
        plt.title(f'Sample {i + 1}', fontsize=10)  # 缩小标题字体
        plt.xlabel('Time', fontsize=8)
        plt.ylabel('Amplitude', fontsize=8)
        plt.tick_params(axis='both', labelsize=8)  # 缩小刻度标签

    # 手动调整布局代替tight_layout
    plt.subplots_adjust(
        left=0.1,  # 左边距
        right=0.9,  # 右边距
        top=0.92,  # 上边距
